...
#include "ui_zOrboxcontroldialog.h"
...
std::list<std::string> ZOrboxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZOrboxControlDialog::listMints;

ZOrboxControlDialog::ZOrboxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZOrboxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZOrboxControlDialog::~ZOrboxControlDialog()
{
    delete ui;
}

void ZOrboxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOrboxControlDialog::updateList()
{
...
void ZOrboxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOrboxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZOrbox_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOrboxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZOrboxControlDialog::GetSelectedMints()
{
...
void ZOrboxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
