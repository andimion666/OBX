                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zOBX ought to be enough for anybody." - Bill Gates, 2017
    ui->zOBXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzOBXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzOBX->setEnabled(false);
        ui->pushButtonMintzOBX->setToolTip(tr("zOBX is currently disabled due to maintenance."));

        ui->pushButtonSpendzOBX->setEnabled(false);
        ui->pushButtonSpendzOBX->setToolTip(tr("zOBX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zOBXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzOBX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOBX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zOBX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zOBX in ") +
...
void PrivacyDialog::on_pushButtonSpendzOBX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOBX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zOBX
        sendzOBX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zOBX
    sendzOBX();
}

void PrivacyDialog::on_pushButtonZOrboxControl_clicked()
{
    ZOrboxControlDialog* zOrboxControl = new ZOrboxControlDialog(this);
    zOrboxControl->setModel(walletModel);
    zOrboxControl->exec();
}

void PrivacyDialog::setZOrboxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzOrboxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzOBX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Orbox Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zOBXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zOBX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " OBX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zOBX</b>";
    ...
    // use mints from zOrbox selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZOrboxControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZOrboxControlDialog::GetSelectedMints();
    }
    ...
    // Clear zOrbox selector in case it was used
    ZOrboxControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zOrbox Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Orbox, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zOrbox Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOBX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zOBX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zOBX "));
    ui->labelzOBXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzOBXSyncStatus->setVisible(fShow);
}
